...
function s.cfilter(c,e,tp)
	...
	and c:IsSetCard(0xf72) and c:IsReason(REASON_COST)
end
...
function s.retcon(e,tp,eg,ep,ev,re,r,rp)
	return re and re:GetHandler():IsSetCard(0xf72) and eg:IsExists(s.cfilter,1,nil,tp,rp)
end
...
function s.retop(e,tp,eg,ep,ev,re,r,rp)
	if e:GetHandler():IsRelateToEffect(e) then
		...
		if sg:GetCount()>0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.SelectYesNo(tp,aux.Stringid(id,0)) then
			Duel.BreakEffect()
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
