...
function s.initial_effect(c)
	...
	e3:SetTarget(s.CopySpellNormalTarget)
	e3:SetOperation(s.CopyOperation)
	c:RegisterEffect(e3)
	local e4=e3:Clone()
...
function s.sprcon(e,c)
	...
	return Duel.IsExistingMatchingCard(s.sprfilter,c:GetControler(),LOCATION_MZONE,0,1,nil,c:GetControler())
end
function s.sprop(e,tp,eg,ep,ev,re,r,rp,c)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectMatchingCard(tp,s.sprfilter,tp,LOCATION_MZONE,0,1,1,nil,tp)
...
function s.GetHandEffect(c)
	s.effect_cache=s.effect_cache or {}
	local code=c:GetOriginalCode()
	if s.effect_cache[code] then return s.effect_cache[code] end
	local eset={}
	...
	s.effect_cache[code]=eset
	return eset
end
function s.CheckHandEffect(c,sec,e,tp,eg,ep,ev,re,r,rp)
	local eset=s.GetHandEffect(c)
...
function s.CopySpellNormalFilter(c,sec,e,tp,eg,ep,ev,re,r,rp)
	local te=s.GetHandEffect(c)
	return c:IsType(TYPE_TUNER) and c:IsAttack(0) and c:IsDefense(1800) and c:IsLevel(3)
		and c:IsAbleToGraveAsCost() and c:IsRace(RACE_ZOMBIE) and s.CheckHandEffect(c,sec,e,tp,eg,ep,ev,re,r,rp)
end
function s.CopySpellNormalTarget(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then
	...
	if chk==0 then
		...
		e:SetLabel(0)
		return og:IsExists(s.CopySpellNormalFilter,1,nil,sec,e,tp,eg,ep,ev,re,r,rp)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=og:FilterSelect(tp,s.CopySpellNormalFilter,1,1,nil,sec,e,tp,eg,ep,ev,re,r,rp)
	local _,te,ceg,cep,cev,cre,cr,crp=s.CheckHandEffect(g:GetFirst(),sec,e,tp,eg,ep,ev,re,r,rp)
	local op = 1
	if #te>1 then
		table_des={}
